...
TEST_F(ContainerThreadSafetyTest, ConcurrentReadWrite) {
  ...
  for (int i = 0; i < num_keys; ++i) {
    container->set_typed("key_" + std::to_string(i), i);
  }
    ...
    threads.emplace_back([&, thread_id = i]() {
      ...
      for (int j = 0; j < operations_per_thread && running.load(); ++j) {
        try {
          ...
          container->set_typed(key, thread_id * 1000 + j);
        } catch (...) {
          ++write_errors;
        }
...
TEST_F(ContainerThreadSafetyTest, MemoryPoolStress) {
  ...
  EXPECT_EQ(deallocation_errors.load(), 0)
      << "No deallocation errors should occur";
}
...
TEST_F(ContainerThreadSafetyTest, MixedOperationsStress) {
  ...
  // Pre-populate
  for (int i = 0; i < 50; ++i) {
    container->set_typed("init_" + std::to_string(i), i);
  }
  ...
  for (int i = 0; i < num_worker_threads; ++i) {
    threads.emplace_back([&, thread_id = i]() {
      for (int j = 0; j < operations_per_thread && running.load(); ++j) {
        try {
          std::string key =
              "worker_" + std::to_string(thread_id) + "_" + std::to_string(j);
...
TEST_F(ContainerThreadSafetyTest, ForEachSafety) {
  ...
  // Pre-populate
  for (int i = 0; i < 100; ++i) {
    container->set_typed("item_" + std::to_string(i), i * 2);
  }
  ...
  for (int i = 0; i < num_modifier_threads; ++i) {
    threads.emplace_back([&, thread_id = i]() {
      for (int j = 0; j < iterations && running.load(); ++j) {
        try {
          std::string key =
              "new_" + std::to_string(thread_id) + "_" + std::to_string(j);
          container->set_typed(key, j);
    ...
    threads.emplace_back([&, thread_id = i]() {
      ...
      for (int j = 0; j < operations_per_thread; ++j) {
        try {
          ...
          // Update
          container->set_typed(hot_key, thread_id * 1000 + j);
        } catch (...) {
          ++errors;
        }
      }
      ...
      for (int j = 0; j < operations_per_thread; ++j) {
        try {
          ...
          if (j % 3 == 0) {
            ...
            container->set_typed(key,
                                 std::string("value_" + std::to_string(j)));
          }
...
TEST_F(ContainerThreadSafetyTest, KeysOperationConcurrent) {
  ...
  // Pre-populate
  for (int i = 0; i < 50; ++i) {
    container->set_typed("initial_" + std::to_string(i), i);
  }
  ...
  for (int i = 0; i < num_writer_threads; ++i) {
    threads.emplace_back([&, thread_id = i]() {
      for (int j = 0; j < operations_per_thread && running.load(); ++j) {
        try {
          std::string key =
              "dynamic_" + std::to_string(thread_id) + "_" + std::to_string(j);
          container->set_typed(key, j);
  ...
  for (int iteration = 0; iteration < num_iterations; ++iteration) {
    ...
    for (int i = 0; i < threads_per_iteration; ++i) {
      threads.emplace_back([&, thread_id = i]() {
        for (int j = 0; j < operations_per_thread; ++j) {
          try {
            std::string key = "iter_" + std::to_string(iteration) + "_thread_" +
                              std::to_string(thread_id) + "_op_" +
                              std::to_string(j);
            container->set_typed(key, j * 2);
            container->get(key);
            if (j % 10 == 0) {
              container->remove(key);
            }
