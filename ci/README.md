...
```sh
python3 scripts/check_tag_reality.py --repo kcenon/container_system --tag v1.2.3 \
  --release-commit FULL_RELEASE_MERGE_SHA \
  --port-dir vcpkg-ports/kcenon-container-system
```
...
Tutorial sources previously under `samples/` now live in `examples/tutorials/`.
The existing sample build switches, target names, build binary directory `samples`,
and installed sample destinations are preserved.
