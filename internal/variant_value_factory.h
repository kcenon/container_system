 ...
 * without needing to specify template parameters or handle type conversions
 * manually.
 *
 * Usage examples:
 * ```cpp
 ...
 */
inline value make_null_value(std::string_view name = "") { return value(name); }
...
inline value make_numeric_value(std::string_view name, T val) {
  ...
  if constexpr (std::is_same_v<T, bool>) {
    return make_bool_value(name, val);
  } else if constexpr (std::is_same_v<T, int16_t> || std::is_same_v<T, short>) {
    return make_short_value(name, static_cast<int16_t>(val));
  } else if constexpr (std::is_same_v<T, uint16_t> ||
                       std::is_same_v<T, unsigned short>) {
    return make_ushort_value(name, static_cast<uint16_t>(val));
  } else if constexpr (std::is_same_v<T, int32_t> || std::is_same_v<T, int>) {
    return make_int_value(name, static_cast<int32_t>(val));
  } else if constexpr (std::is_same_v<T, uint32_t> ||
                       std::is_same_v<T, unsigned int>) {
    return make_uint_value(name, static_cast<uint32_t>(val));
  } else if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, long> ||
                       std::is_same_v<T, long long>) {
    return make_long_value(name, static_cast<int64_t>(val));
  } else if constexpr (std::is_same_v<T, uint64_t> ||
                       std::is_same_v<T, unsigned long> ||
                       std::is_same_v<T, unsigned long long>) {
    return make_ulong_value(name, static_cast<uint64_t>(val));
  } else if constexpr (std::is_same_v<T, float>) {
    return make_float_value(name, val);
  } else if constexpr (std::is_same_v<T, double>) {
    return make_double_value(name, val);
  } else {
 ...
 */
inline value make_string_value(std::string_view name, std::string val) {
  return value(name, std::move(val));
}

/**
 * @brief Create a string value from string_view
 */
inline value make_string_value(std::string_view name, std::string_view val) {
  return value(name, std::string(val));
}

/**
 * @brief Create a string value from C-string
 */
inline value make_string_value(std::string_view name, const char *val) {
  return value(name, std::string(val));
}
 ...
 */
inline value make_bytes_value(std::string_view name, const uint8_t *data,
                              size_t size) {
 ...
 * Note: This treats the string as binary data, not as a null-terminated C
 * string
 */
inline value make_bytes_from_string(std::string_view name,
 ...
 */
inline value
make_container_value(std::string_view name,
 ...
 * Prefer type-specific factories (make_int_value, etc.) when the type is known
 * at compile time.
 *
 * @param name Value name
 ...
 */
inline value make_value_from_raw(std::string_view name, value_types type,
 ...
 */
inline value clone_with_name(const value &original, std::string_view new_name) {
